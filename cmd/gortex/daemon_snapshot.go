...
// saveSnapshotMetadata is the persistent-backend counterpart to
// saveSnapshot. It writes a header with NodeCount=0 / EdgeCount=0
// followed by the repos + contracts + vector sections — no graph
// data. Used when the graph already lives in the backend's own
// on-disk store (ladybug), so the snapshot only needs to carry the
// data the backend doesn't persist on its own: per-repo FileMtimes
// (for IncrementalReindex on warm restart), per-repo contract
// registries, and the workspace vector index.
//
// Without this, a persistent-backend daemon restart had no mtimes
// to feed ReconcileRepoCtx, fell through to a full TrackRepoCtx walk
// for every repo, and tripped BulkUpsertSymbolFTS over an already-
// populated FTS index — the bulk-COPY path that crashes on warm
// stores.
func saveSnapshotMetadata(repos []snapshotRepo, snapContracts []snapshotContract, vec snapshotVector, version string, logger *zap.Logger) {
	// Ladybug backend: write to the per-backend path so the memory
	// backend can't load this metadata-only file and end up with an
	// empty graph. See daemon.BackendSnapshotPath.
	_ = saveSnapshotMetadataTo(repos, snapContracts, vec, version, daemon.BackendSnapshotPath("ladybug"), logger)
}

// saveSnapshotMetadataTo is saveSnapshotMetadata with an explicit path
// argument, mirroring the saveSnapshotTo / saveSnapshot split on the
// graph-bearing side.
func saveSnapshotMetadataTo(repos []snapshotRepo, snapContracts []snapshotContract, vec snapshotVector, version string, path string, logger *zap.Logger) error {
	if err := daemon.EnsureParentDir(path); err != nil {
		logger.Warn("snapshot: parent dir", zap.Error(err))
		return err
	}
	...
	// Skip snapshotWouldCollapse — that heuristic is keyed off
	// node/edge counts which are intentionally zero here.
	if err := os.Rename(tmp, path); err != nil {
		logger.Warn("snapshot: rename", zap.Error(err))
		return err
	}
	logger.Info("snapshot: wrote (metadata-only)",
		zap.String("path", path),
		zap.Int("repos", header.RepoCount),
		zap.Int("contracts", header.ContractCount),
		zap.Int("vectors", header.VectorCount))
...
// loadSnapshotMetadata is the persistent-backend counterpart to
// loadSnapshot. It reads the header + repos + contracts + vector
// sections and silently skips any node/edge records the snapshot
// happens to carry (a snapshot written by a memory-backend daemon
// before a switch to ladybug is the realistic source of non-zero
// counts; throwing those rows on the floor is correct because the
// persistent backend already has the authoritative graph state).
func loadSnapshotMetadata(logger *zap.Logger) (snapshotLoadResult, error) {
	// Ladybug warm-restart reads from its own backend-tagged path.
	// Falls back to the legacy unsuffixed daemon.gob.gz when the new
	// file is absent — covers users upgrading from before the per-
	// backend split.
	res, err := loadSnapshotMetadataFrom(daemon.BackendSnapshotPath("ladybug"), logger)
	if err == nil && (res.Loaded || res.Partial) {
		return res, nil
	}
	return loadSnapshotMetadataFrom(daemon.SnapshotPath(), logger)
}

func loadSnapshotMetadataFrom(path string, logger *zap.Logger) (snapshotLoadResult, error) {
	result := snapshotLoadResult{
		Contracts: make(map[string][]contracts.Contract),
	}
	f, err := os.Open(path)
	...
	// Metadata-only loads skip the binary-version + binary-mtime
	// discard gates that the full loadSnapshotFrom enforces. Those
	// gates exist to invalidate persisted resolver state across
	// daemon rebuilds — but the metadata-only payload carries no
	// resolved edges (the graph lives in the backend store). The
	// mtimes themselves are immune to resolver changes; the worst
	// case if a few mtimes are off is that IncrementalReindex
	// re-indexes a handful of extra files, which is what we want
	// during recovery. Discarding the whole payload over a binary
	// rebuild was the original cause of warm-restart falling back to
	// the bulk-COPY crash path.
	result.Vector = snapshotVector{
		Index: header.VectorIndex,
		Dims:  header.VectorDims,
		Count: header.VectorCount,
	...
	// Discard any node/edge records the snapshot carries. The backend
	// already owns the graph; replaying nodes/edges here would either
	// be a no-op (idempotent MERGE) or duplicate writes — both
	// expensive. Decoding into a throwaway struct keeps the gob
	// stream's record-by-record positional contract intact so the
	// repos/contracts sections that follow still decode cleanly.
	for i := 0; i < header.NodeCount; i++ {
		var n graph.Node
		if err := dec.Decode(&n); err != nil {
			if errors.Is(err, io.EOF) || errors.Is(err, io.ErrUnexpectedEOF) {
				logger.Warn("snapshot: truncated during nodes (metadata load)",
					zap.Int("expected", header.NodeCount),
					zap.Int("read", i),
					zap.Error(err))
				return result, nil
			}
			// One bad record: keep going, the stream stays positional
			// (gob skips the malformed record's bytes internally).
			continue
		}
	}
	for i := 0; i < header.EdgeCount; i++ {
		...
		if err := dec.Decode(&e); err != nil {
			if errors.Is(err, io.EOF) || errors.Is(err, io.ErrUnexpectedEOF) {
				logger.Warn("snapshot: truncated during edges (metadata load)",
					zap.Int("expected", header.EdgeCount),
					zap.Int("read", i),
					zap.Error(err))
				return result, nil
	...
	if header.RepoCount > 0 {
		...
		for i := 0; i < header.RepoCount; i++ {
			...
			if err := dec.Decode(&r); err != nil {
				if errors.Is(err, io.EOF) || errors.Is(err, io.ErrUnexpectedEOF) {
					logger.Warn("snapshot: truncated during repos (metadata load)",
						zap.Int("expected", header.RepoCount),
						zap.Int("read", i),
						zap.Error(err))
					return result, nil
	...
	if header.ContractCount > 0 {
		for i := 0; i < header.ContractCount; i++ {
			...
			if err := dec.Decode(&sc); err != nil {
				if errors.Is(err, io.EOF) || errors.Is(err, io.ErrUnexpectedEOF) {
					logger.Warn("snapshot: truncated during contracts (metadata load)",
						zap.Int("expected", header.ContractCount),
						zap.Int("read", i),
						zap.Error(err))
					return result, nil
	...
	totalRepos := len(result.Repos)
	totalContracts := 0
	for _, cs := range result.Contracts {
		totalContracts += len(cs)
	}
	logger.Info("snapshot: loaded (metadata-only)",
		zap.String("path", path),
		zap.Int("repos", totalRepos),
		zap.Int("contracts", totalContracts),
		zap.Int("vectors", result.Vector.Count))
	result.Loaded = true
	return result, nil
}
