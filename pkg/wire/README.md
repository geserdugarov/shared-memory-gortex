...
- **License:** MIT (this package only; the rest of Gortex ships under a
  separate source-available license — see `LICENSE.md` at the repo root).
...
Every GCX1 payload decodes back to an equivalent JSON value. Rerun the
benchmark (`go run ./bench/wire-format` from the parent repo) to verify
20/20 round-trip integrity on the reference fixture set.
